#!/usr/bin/env bash
#SBATCH --job-name=smolvla_robocerebra
...
WT=/fsx/pepijn/lerobot_wt/robocerebra
BRANCH=feat/robocerebra-benchmark
...
prefetch_hf_dataset "pepijn223/robocerebra_unified_v3"

accelerate launch --num_processes=8 \
  -m lerobot.scripts.lerobot_train \
  --policy.path=lerobot/smolvla_base \
  --policy.repo_id=pepijn223/smolvla_robocerebra \
  --policy.load_vlm_weights=true \
  --policy.scheduler_decay_steps=20000 \
  --policy.freeze_vision_encoder=false \
  --policy.train_expert_only=false \
  --dataset.repo_id=pepijn223/robocerebra_unified_v3 \
  --dataset.revision=main \
  --dataset.use_imagenet_stats=false \
  --env.type=robocerebra \
  --env.task=libero_10 \
  --output_dir=outputs/train/smolvla_robocerebra \
  --steps=20000 \
  --batch_size=32 \
  --eval_freq=0 \
  --save_freq=20000 \
